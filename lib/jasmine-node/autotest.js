...
// On Windows, fs.watch({recursive}) asserts in libuv when handed an 8.3 short
// path, so resolve to the canonical long path before watching.
var canonicalPath = function(p) {
    try {
...
exports.start = function(loadpaths, watchFolders, patterns) {
    loadpaths.forEach(function(loadpath){
      ...
      // fs.watch never emits an initial event, so chokidar's ignoreInitial is moot.
      fs.watch(canonicalPath(loadpath), {recursive: true}, onChanged);
    ...
    watchFolders.forEach(function(watchPath) {
      ...
      fs.watch(canonicalPath(watchPath), {recursive: true}, onChanged);
    });
